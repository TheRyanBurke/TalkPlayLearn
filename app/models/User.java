...
public class User extends Model {
	...
	/**
	 * Similar to a character sheet in an RPG Other members can freely award
	 * stat points to others Some quests will also boost stats
	 * 
	...
	private void populateStats() {
		stats.put(Constants.CREATIVITY, 0);
		stats.put(Constants.ENTHUSIASM, 0);
		stats.put(Constants.ACADEMIC, 0);
		stats.put(Constants.PRODUCTIVITY, 0);
		stats.put(Constants.GAMER, 0);
		stats.put(Constants.SOCIALNESS, 0);
	}
			...
			if (eq.quest.equals(search)) {
				...
				if (eq.completed) {
					if (search.repeatability == Quest.REPEATABLE.DAILY) {
						if (new GregorianCalendar().getTime().getTime()
								- eq.completedOn.getTime() < 86400000L) {
							return false;
						}
	...
	public void tickObjective(long engagedQuestId, long objectiveId,
			boolean uptick) {
